...
import com.cszczotka.http.api.handler.{AuthenticationHandler, JwtApiHandler}
...
trait JwtApi extends JwtApiHandler with AuthenticationHandler {
  ...
  val routes: Route =
    get {
        ...
        path("getUserDetail" / IntNumber) { (userId: Int) =>
          ctx: RequestContext =>
            val r: String = if (isVerify(ctx.request, secretKey)) {
              val allUsersName: (String, String, Int) = getUserInfoById(userId)
              s"user names are  ${allUsersName.toString()} "
            } else {
              "other users......"
            }
          ...
          entity(as[Multipart.FormData]) { data =>
            complete {
              ...
              extractedData.map { data =>
                ...
                val res = if (result._1 == userName && checkHash(password, result._2)) {
      ...
      path("login_ajax") {
        post {
          parameters('user, 'password) { (login, password) =>
            println(login + "@" + password)
            complete(StatusCodes.OK)
          }
        }
      } ~
      (path("login_ajax3") & post) { ctx: RequestContext =>
